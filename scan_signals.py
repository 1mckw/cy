#!/usr/bin/env python3
"""Hyperliquid perpetual TOP50 — AR/AD + trend-line scanner (1H / 4H / 1D).
...
def render_html(payload: dict) -> str:
    ...
    def row_ar_dr(h: dict) -> str:
        ...
        return (
            ...
            f'<td><span class="tag {cls}">{html.escape(signal_type_label(str(h.get("type", ""))))}</span></td>'
            f"<td>{tf_cell(h)}</td>"
            f"<td>{pool_cell(h)}</td>"
            f"<td>{sym_btn(h)}</td>"
...
<body>
  <div class="wrap">
    ...
    <h2>AR / AD 晚觸碰（最近 10 根日 K 內 · 超過 20 根後 · 根數 ≥ 60）</h2>
    <div class="panel"><table><thead><tr>
      <th>類型</th><th>週期</th><th>池</th><th>代碼</th><th>名稱</th><th class="num">價位</th><th class="num">根數</th><th>時間</th>
